...
export default function MangaReader() {
  ...
  const [currentMangaFolder, { mutate: setCurrentMangaFolder, refetch: refetchCurrentMangaFolder }] = createResource(folderPath, get_os_folder_by_path);
  ...
  createEffect(() => {
    if (currentMangaFolder.state === "ready" && panels.state === "ready" && !hasInitialized()) {
        ...
        <Show when={currentMangaFolder.state === "ready"}>
          <Show when={panels.state === "ready" && user.state === "ready"}>
            <div class="h-full w-full flex justify-center items-center pb-8">
              <div
                class="h-[96.6%] lg:px-4 xl:px-10 w-auto z-20 absolute left-0 flex items-center cursor-pointer
              ...
              <div
                class="h-[96.6%] lg:px-4 xl:px-10 w-auto z-20 absolute right-0 flex items-center cursor-pointer
							justify-center hover:bg-primary/15 transition-all opacity-0 hover:opacity-80"
                onClick={async () => {
                  if (isDoublePanels()) {
              ...
              <h2
                class="text-nowrap rounded-b-sm hover:shadow-md hover:shadow-primary/15
								select-none text-accent/5 font-medium px-3 z-50 h-fit pb-0.5
								hover:text-secondary hover:bg-primary transition-all duration-300"
                style={{
                  position: "absolute",
                  top: "29px",
                  left: "50%",
              ...
              <div class="relative flex flex-row justify-center items-center">
                ...
                <Show when={PREV_PANELS().first && PREV_PANELS().second && panelIndex() - 2 >= 1}>
                  <div class="absolute left-0 top-0 flex transition-opacity duration-0" style={{ opacity: 0.001 }}>
