...
import { IconBookFilled, IconFolderFilled } from "@tabler/icons-solidjs";
...
export const OsFolderDescription: Component<{ folder: () => OsFolder }> = (
  props,
) => {
  const [lrp_title, lrp_ext] = splitTitleDots(
    props.folder()?.last_read_panel?.title,
  );
      ...
      <div class="flex flex-col">
        ...
        >
          ...
          <Show when={props.folder().is_read}>
            <IconBookFilled class="h-4 transition-all duration-200 fill-secondary/80 dark:fill-secondary-foreground/80" />
          </Show>
        ...
        <div class="mt-1">
          <Show when={props.folder().metadata?.contains.folders > 0}>
            ...
            >
              ...
              <IconFolderFilled class="h-auto w-3 pl-0.5" />
            </p>
          </Show>
          <p
