...
export default function CreateProfile() {
  const initialUser: UserFormType = {
    username: "" /* add future fields here */,
  };
  ...
  const handleSubmit = (event: Event) => {
    ...
    invoke("update_user", { user: dbUser })
      .then(() => {
        navigate("/dashboard");
      })
      .catch((e) => {
      ...
      <Card class="flex flex-col p-4">
        ...
        >
          <div class="flex items-center space-x-6">
            <CardTitle class="rounded-sm bg-primary px-1.5 text-xl text-secondary shadow-md">
              Customize Profile
            </CardTitle>
            <ImageRoot
              ...
              <For each={Object.keys(initialUser) as (keyof UserFormType)[]}>
                {(field) => (
                  <div class="flex flex-col">
                    <label
                      for={field}
                      class="ml-0.5 text-sm font-medium text-zinc-200"
                    >
                    ...
                    <input
                      ...
                      value={user()[field] || ""}
                      onInput={(e) =>
                        handleInputChange(field, e.currentTarget.value)
                      }
